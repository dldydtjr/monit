package com.dev.nao.monitoring.web;
...
//		}
	...
	@RequestMapping(value = "/monitoring/getData", method = RequestMethod.POST)
	public String getData(@ModelAttribute("monitoringVO") MonitoringVO monitoringVO,
...
//		        		  };
		...
		List<MonitoringVO> list = monitoringService.data(monitoringVO);
...
//			if(labels[j].equals(monitoringVO.getSearchTime())) {
//				break;
//			}
		}
//		int cnt = 0;
...
//				if(labels[i].equals(list.get(j).getLabels())) {
//					value = Double.valueOf(list.get(j).getPtValue());
//					hisDate = list.get(j).getHistDate();
//					valueChk++;
//					continue;
//				}
//			}
...
//				cnt = 0;
//			}
//			if(labels[i].equals(monitoringVO.getSearchTime())) {
//				break;
//			}
//		}
		jo.put("data", ja);
...
//		jo.put("date", ja3);
	...
	@RequestMapping(value = "/monitoring_hist", method = RequestMethod.GET)
	public String monitoring_hist(HttpServletRequest request, Model model) throws Exception {
		if(request.getSession().getAttribute("id") == null) {
			return "redirect:/";
		}
		
		return "monitoring/monitoring_hist";
	}
	
	@RequestMapping(value = "/monitoring/getData_hist", method = RequestMethod.POST)
	public String getData_hist(@ModelAttribute("monitoringVO") MonitoringVO monitoringVO,
			HttpServletRequest request, Model model) throws Exception {
		...
		List<MonitoringVO> list = monitoringService.data_hist(monitoringVO);
		for(int i=0; i<list.size();i++) {
			ja.put(i, list.get(i).getPtValue());
			ja2.put(i, list.get(i).getLabels());
			ja3.put(i, list.get(i).getHistDate());
		}
		jo.put("data", ja);
		jo.put("time", ja2);
		jo.put("date", ja3);
